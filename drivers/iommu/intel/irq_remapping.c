...
static struct intel_ir_data *find_detached_irte(struct irq_alloc_info *info,
						int sub_index)
{
	struct pci_dev *pdev = info->msi_dev;
	u16 msi_desc_index;
	struct intel_ir_data *ird;
	u16 bdf;

	/* see pci_msi_domain_calc_hwirq() */
	msi_desc_index = info->msi_hwirq & 0x7FF;
			 ...
			 struct irq_alloc_info *info)
{
	...
	for (i = 0; i < nr_irqs; i++) {
		...
		ird->irq_2_dev.pdev = info->msi_dev;
				     ...
				     void *arg)
{
	...
	for (i = 0; i < nr_irqs; i++) {
		...
		if (pdev) {
			...
			ird->irq_2_dev.msi_desc_index = info->msi_hwirq & 0x7FF;
